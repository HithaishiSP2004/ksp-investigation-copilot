...
import { OfficerBriefing } from "./officer-briefing";
...
export function CopilotView({ initialCaseId, onCaseChange }: CopilotViewProps) {
  const { t } = useLocale();
  const [cases, setCases] = useState<CaseDetailsUI[]>([]);
  const [briefing, setBriefing] = useState<BriefingData | null>(null);
  const [graph, setGraph] = useState<{ renderNodes: RenderNode[]; renderLinks: RenderLink[] } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      KnowledgeService.getBriefing(initialCaseId),
      KnowledgeService.getGraph(initialCaseId),
    ])
      .then(([b, g]) => {
        if (active) {
          setBriefing(b);
  ...
  return (
    <div className="space-y-4">
      ...
      {initialCaseId ? (
        isLoading ? (
          <div className="h-[40vh] flex items-center justify-center bg-card border border-border rounded-xl">
            <div className="flex flex-col items-center gap-2">
              <div className="h-6 w-6 animate-spin rounded-full border-2 border-primary border-t-transparent" />
              <span className="text-xs text-muted-foreground font-semibold">{t("loadingBriefing")}</span>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            <div className="lg:col-span-1">
              {briefing && <OfficerBriefing data={briefing} />}
            </div>
            <div className="lg:col-span-2">
              {graph && <RelationshipGraph nodes={graph.renderNodes} links={graph.renderLinks} />}
            </div>
          </div>
        )
      ) : (
